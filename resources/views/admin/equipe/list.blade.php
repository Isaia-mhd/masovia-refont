...
@section('content')
    <div class="w-full px-4 sm:px-6 md:px-8 lg:px-12 xl:px-20 max-w-5xl mx-auto bg-[#27283E] pt-12 pb-10">
        <div class="mb-10 text-center">
            ...
            <h2 class="text-white text-xl font-semibold">Tous les équipes de Masovia</h2>
        </div>

        <div class="w-full bg-[#27283E] shadow-2xl shadow-[#17182c] rounded-lg px-4 sm:px-8 lg:px-12 py-6 ">
            <h2 class="text-white text-xl sm:text-2xl mb-1">Liste d'équipe</h2>
            <p class="text-[#C0C0C0] text-xs sm:text-sm mb-4">
                Voici les listes des équipes de Masovia Madagascar.
            </p>
            ...
            <div class="space-y-1">
                @foreach ($teams as $team)
                    @include('components.equipe_liste_card', ['team' => $team])
                @endforeach
            </div>
        </div>

        <div class="w-15 h-15 flex justify-center items-center rounded-full bg-[#F0F235] fixed bottom-10 right-10">
            <a href="{{ route("equipe.nouveau") }}"><i class="fa fa-solid fa-add text-gray-slate-700 font-bold"></i></a>
        </div>
    </div>
@endsection
